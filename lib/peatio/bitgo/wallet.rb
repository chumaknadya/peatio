  ...
  class Wallet < Peatio::Wallet::Abstract
    ...
    def create_address!(options = {})
      if options[:bitgo_address_id].present?
        response = client.rest_api(:get, '/wallet/' + wallet_id + '/address/' + options[:bitgo_address_id])
        { address: response['address'] }
      else
        response = client.rest_api(:post, 'ff/wallet/' + wallet_id + '/address')
    ...
    def create_transaction!(transaction, options = {})
      ...
      if currency_options[:gas_limit].present? && currency_options[:gas_price].present?
        ...
        txid = client.rest_api(:post, '/wallet/' + wallet_id + '/sendcoins', {
    ...
    def build_raw_transaction(transaction)
      client.rest_api(:post, '/wallet/' + wallet_id + '/tx/build', {
        recipients: [{
    ...
    def load_balance!
      response = client.rest_api(:get, "/wallet/#{wallet_id}")
